...
Write-Host "Creating Ergobaby Omni 360 product in Sanity..." -ForegroundColor Green
Write-Host ""
...
try {
    ...
    if ($Result.results -and $Result.results[0].id) {
        Write-Host "SUCCESS! Product created in Sanity" -ForegroundColor Green
        Write-Host "Document ID: $($Result.results[0].id)" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "Status: DRAFT (unpublished)" -ForegroundColor Yellow
        Write-Host "Next: Log into Sanity Studio, add product image, and publish" -ForegroundColor Yellow
    }
    else {
