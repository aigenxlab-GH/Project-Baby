  ...
  return (
    <>
      ...
      <nav
        ...
        className="hidden xl:block sticky top-24 bg-gray-50 border border-gray-200 rounded-2xl p-5 max-h-[70vh] overflow-y-auto"
      >
        <div className="flex items-center gap-2 mb-4">
          <List className="h-4 w-4 text-brand-600" aria-hidden="true" />
          <h2 className="font-semibold text-gray-900 text-sm">In this article</h2>
        </div>
        <ol className="space-y-1">
          {items.map((item) => (
            <li key={item.id}>
              ...
              >
                {item.level === 3 && <span className="text-gray-300 mr-1">└</span>}
                {item.text}
              </button>
            </li>
          ))}
      ...
      <div className="xl:hidden mb-6 bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden">
        <button
        ...
        {mobileOpen && (
          <ol id="toc-mobile-list" className="px-5 pb-4 space-y-1 border-t border-gray-100">
            {items.map((item) => (
              <li key={item.id}>
                <button
                  onClick={() => scrollTo(item.id)}
                  className={`text-left w-full text-sm py-1.5 text-brand-600 hover:underline ${item.level === 3 ? 'pl-4' : ''}`}
                >
                  {item.text}
                </button>
              </li>
