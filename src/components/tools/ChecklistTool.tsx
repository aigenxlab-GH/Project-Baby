...
function trackChecklistClick(source: string, itemName: string, href: string) {
...
export function ChecklistTool({ items, categoryLabels, progressLabel, trackingSource }: Props) {
  ...
  const filtered = items.filter((item) => filter === 'all' || item.priority === filter);
  const categories = [...new Set(filtered.map((i) => i.category))];
  const totalChecked = filtered.filter((i) => checked.has(i.id)).length;
  const progressPct = filtered.length ? Math.round((totalChecked / filtered.length) * 100) : 0;
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <p className="text-sm text-gray-500">{totalChecked} / {filtered.length} items</p>
        </div>
      ...
      <fieldset className="mb-6">
        <legend className="sr-only">Filter checklist items by priority</legend>
                  ...
                  <div key={item.id} className={`flex items-start gap-4 px-5 py-4 transition-colors ${isChecked ? 'bg-green-50' : 'hover:bg-gray-50'}`}>
                    ...
                    {item.affiliateUrl && (
                      <a
                        ...
                        onClick={() => trackChecklistClick(trackingSource, item.name, item.affiliateUrl!)}
