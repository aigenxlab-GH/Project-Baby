...
export function MedicalDisclaimer({ variant = 'banner', className = '' }: Props) {
    ...
    return (
      <div className={`bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6 ${className}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-amber-900 font-semibold mb-1">Medical Information</p>
  ...
  return (
    <div className={`bg-red-50 border-l-4 border-red-500 p-4 mb-8 ${className}`} role="alert" aria-live="polite">
      <div className="flex items-start gap-3">
        <AlertCircle className="h-6 w-6 text-red-600 mt-0.5 flex-shrink-0" />
        <div>
          <h3 className="font-semibold text-red-900 text-sm mb-1">Medical Disclaimer</h3>
          ...
          <p className="text-xs text-red-700 mt-2 font-medium">
            ⚠️ If you are experiencing an emergency, call 999 (UK) or 911 (US) immediately.
          </p>
        </div>
      </div>
    </div>
