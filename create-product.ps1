...
$Payload = $ProductData | ConvertTo-Json -Depth 10
$MutationPayload = @{ mutations = @(@{ create = $ProductData }) } | ConvertTo-Json -Depth 10

$Uri = "https://$ProjectId.sanity.io/v2021-06-07/data/mutate/$Dataset"
...
Write-Host "Creating Ergobaby Omni 360 product..."
Write-Host ""

try {
    $Response = Invoke-WebRequest -Uri $Uri -Method Post -Headers $Headers -Body $MutationPayload -ErrorAction Stop
    $Result = $Response.Content | ConvertFrom-Json

    Write-Host "SUCCESS: Product created!"
    Write-Host "Document ID: $($Result.results[0].id)"
    Write-Host ""
    Write-Host "Status: DRAFT (unpublished)"
    Write-Host "Next: Log into Sanity Studio, add image, and publish"
}
catch {
